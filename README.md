...
- [COSAI-Labs/toolpipe-mcp-server](https://github.com/COSAI-Labs/toolpipe-mcp-server) 📇 ☁️ 🏠 🍎 🪟 🐧 - 120+ developer utility tools via MCP: code review, fake data generation, JSON/CSV/XML processing, QR codes, hashing, DNS lookup, SSL checking, WHOIS, Docker Compose generation, GitHub Actions workflows, Nginx configs, API client generation, and more. Remote (Streamable HTTP) and local (npx) modes.
